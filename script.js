...
// Process steps with timing (in seconds for demo)
...
function startCountdown(){
    ...
    // Update timer display intitally
    updateTimerDisplay();
...
function startPreparation(){
    ...
    function processNextStep(){
        ...
        if(currentStep < processSteps.length - 1){
            ...
            addUpdate("Timer Method",
                `Using setTimeout(${currentDuration * 1000}) to simulate "${processSteps[currentStep].name}" stage (${currentDuration / 60} minutes in real life)`);
    ...
    addUpdate("Timer Method",
        `Using setTimeout(${processSteps[0].duration * 1000}) to simulate moving to "${processSteps[1].name}" stage (${processSteps[0].duration / 60} minutes in real life)`);
...
function getStatusMessage(stepIndex){
    const messages = [
        ...
        "Your pizza is now baking in out brick oven at 700 degrees fahrenheit.",
    ...
    console.log(messages[stepIndex])
